...
public class OrcamentoFormView extends CustomComponent {
	...
	ComboBox cmbVendedor, cmbCliente, cmbTransportadora, cmbCondicaoPagamento,
			cmbTipo, cmbFrete;
	...
	private GridLayout buildFields() {
		...
		cmbCondicaoPagamento = ComponentUtil
				.buildComboBox("Condição Pagamento");
		cmbCondicaoPagamento.setContainerDataSource(controller
				.carregarCondicoes());
	...
	private TabSheet buildItensSubForm() {
		...
		itensSubForm = new SubFormComponent<ItemOrcamento, Integer>(
				ItemOrcamento.class, new String[] { "produto", "quantidade",
						"valorUnitario", "valorSubTotal", "taxaDesconto",
						"valorDesconto", "valorTotal" }, new String[] {
						"Produto", "Quantidade", "Valor Unitário",
						"Valor SubTotal", "Taxa Desconto", "Valor Desconto",
			...
			protected TableFieldFactory getFieldFactory() {
				return new TableFieldFactory() {
					...
					public Field<?> createField(Container container,
						...
						// BeanItemContainer<String> beanUf = new
						// BeanItemContainer<>(
						// String.class);
						// for (UF u : ufs) {
						...
						if ("produto".equals(propertyId)) {
							ComboBox field = ComponentUtil
									.buildComboBox("Produto");
							field.setContainerDataSource(controller
									.carregarProdutos());
							return field;
						}
						...
						if ("quantidade".equals(propertyId)) {
							TextField field = ComponentUtil
									.buildNumberField("Quantidade");
							field.addBlurListener(getBlurListener(container,
									itemId, propertyId));
							return field;
						}
						...
						// TextField field =
						// ComponentUtil.buildCurrencyField("Valor SubTotal");
						// field.addBlurListener(getBlurListener(container,
						// itemId, propertyId));
						// return field;
						// }
						...
						if ("taxaDesconto".equals(propertyId)) {
							TextField field = ComponentUtil
									.buildNumberField("Taxa Desconto");
							field.addBlurListener(getBlurListener(container,
									itemId, propertyId));
							return field;
						}
						...
						if ("valorDesconto".equals(propertyId)) {
							TextField field = ComponentUtil
									.buildCurrencyField("Valor Desconto");
							field.addBlurListener(getBlurListener(container,
									itemId, propertyId));
							return field;
						}
