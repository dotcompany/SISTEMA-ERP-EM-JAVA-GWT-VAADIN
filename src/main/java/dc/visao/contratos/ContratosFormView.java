				...
				return new TableFieldFactory() {
					...
					public Field<?> createField(Container container, Object itemId, Object propertyId, Component uiContext) {
						...
						if ("produto.nome".equals(propertyId)) {
							...
							// Foi necessario criar um ComboBox, pois o metodo retorna um Field
							// Carrega ja os produtos listados, e seta no ComboBox criado
						    List<Produto> list = cmbProduto.getModel().getAll();
						    ComboBox cmbProdutoObjeto = new ComboBox();
						    //BeanContainer<String, ComboItemValue> objects = new BeanContainer<String, ComboItemValue>(ComboItemValue.class);
						    //objects.setBeanIdProperty("id");
						    ...
						    for (Produto val : list) {
						    	...
						    	item.setBean(val);
						    	//objects.addBean(new ComboItemValue(val.getId(), val.getNome()));
						    	objects.addBean(item);
						    	//cmbProdutoObjeto.addItem(item);
						    }
							...
							//cmbProdutoObjeto.setConverter((Converter)new StringtoRowIdConverter());
