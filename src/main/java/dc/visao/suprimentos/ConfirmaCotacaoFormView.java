...
public class ConfirmaCotacaoFormView extends CustomComponent {
	...
	private GridLayout buildFields() {
		...
		cmbFornecedor.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				FornecedorCotacao fornCotacao = (FornecedorCotacao) cmbFornecedor.getValue();
				if (oldValue != null) {
					oldValue.setPrazoEntrega(txtPrazoEntrega.getValue());
					oldValue.setVendaCondicoesPagamento(txtCondicaoPagamento.getValue());
					oldValue.setValorSubtotal((BigDecimal) txtSubtotal.getConvertedValue());
					oldValue.setTaxaDesconto((BigDecimal) txtTaxaDesconto.getConvertedValue());
					oldValue.setValorDesconto((BigDecimal) txtValorDesconto.getConvertedValue());
					oldValue.setTotal((BigDecimal) txtTotal.getConvertedValue());
				}
				
				if (fornCotacao != null) {
					fillReqCotacaoDetalhesSubForm(fornCotacao.getCotacaoDetalhes());
					txtPrazoEntrega.setValue(fornCotacao.getPrazoEntrega());
					txtCondicaoPagamento.setValue(fornCotacao.getVendaCondicoesPagamento());
					txtSubtotal.setConvertedValue(fornCotacao.getValorSubtotal());
					txtTaxaDesconto.setConvertedValue(fornCotacao.getTaxaDesconto());
					txtValorDesconto.setConvertedValue(fornCotacao.getValorDesconto());
					txtTotal.setConvertedValue(fornCotacao.getTotal());
				} else {
					fillReqCotacaoDetalhesSubForm(new ArrayList<CotacaoDetalhe>(0));
		...
		txtCondicaoPagamento = ComponentUtil.buildTextField("condição de Pagamento");
		...
		txtValorDesconto = ComponentUtil.buildCurrencyField("Valor de Desconto");
		...
		cotacaoDetalheSubForm = new SubFormComponent<CotacaoDetalhe, Integer>(CotacaoDetalhe.class, new String[] { "produto.nome",
				"quantidade", "valorUnitario", "valorSubtotal", "taxaDesconto", "valorDesconto", "valorTotal" }, new String[] {
				"Produto requisitado", "Quantidade", "Valor Unitário", "Valor SubTotal", "Taxa Desconto", "Valor Desconto",
				"Valor Total" }, new String[] { "valorSubtotal", "valorDesconto", "valorTotal" }) {
			@Override
			protected TableFieldFactory getFieldFactory() {
				return new TableFieldFactory() {
					...
					public Field<?> createField(Container container, Object itemId, Object propertyId, Component uiContext) {
						if ("quantidade".equals(propertyId) || "produto.nome".equals(propertyId)) {
							TextField textField = ComponentUtil.buildTextField(null);
							textField.setReadOnly(true);
							return textField;
						} else if ("valorUnitario".equals(propertyId) || "valorDesconto".equals(propertyId)) {
							TextField buildCurrencyField = ComponentUtil.buildCurrencyField(null);
							buildCurrencyField.addBlurListener(getBlurListener(container, itemId, propertyId));
							return buildCurrencyField;
						} else if ("valorSubtotal".equals(propertyId) || "valorTotal".equals(propertyId)) {
							TextField buildCurrencyField = ComponentUtil.buildCurrencyField(null);
			...
			protected CotacaoDetalhe getNovo() {
				throw new UnsupportedOperationException("Não é possível criar novo!");
			}
	...
	public void fillReqCotacaoDetalhesSubForm(List<CotacaoDetalhe> requisicaoDetalhes) {
		cotacaoDetalheSubForm.fillWith(requisicaoDetalhes);
	}

	public void fillCompraFornecedorCotacoesSubForm(List<FornecedorCotacao> lista) {
		BeanItemContainer<FornecedorCotacao> tipoRequisicaoContainer = new BeanItemContainer<>(FornecedorCotacao.class, lista);
		tipoRequisicaoContainer.addNestedContainerProperty("fornecedor.pessoa.nome");
