			...
			if(valido){
				...
				if(subView.getTfLucroServico()!=null){
					...
					if (Validator.validateString(lucroServico)) {
						...
						this.currentBean.setValorLucroParcial(new BigDecimal(lucroServico));
					}
				}
...
//					subView.getBtnEfetivacao().setCaption("Financeiro");
	...
	public void salvarInformacaoGeral() {
		InformacaoGeralEntity informacaoGeral = informacaoGeralDAO.buscaInformacaoGeral(currentBean);
	...
	public void salvarLaudoTecnico() {
		LaudoTecnicoEntity laudoTecnico = laudoTecnicoDAO.buscaLaudoTecnico(currentBean);
	...
	private void preencheCombos() {
		...
		DefaultManyToOneComboModel<StatusOsEntity> statusOs = new DefaultManyToOneComboModel<StatusOsEntity>(StatusOsListController.class, this.statusOsDAO,
				super.getMainController()) {
			@Override
			public String getCaptionProperty() {
				return "descricao";
		...
		DefaultManyToOneComboModel<SituacaoServicoEntity> situacaoServico = new DefaultManyToOneComboModel<SituacaoServicoEntity>(
...
//		this.subView.getCbCorGarantia().setModel(cor);
	...
	public void carregarInformacaoGeral() {
		InformacaoGeralEntity info = informacaoGeralDAO.buscaInformacaoGeral(currentBean);
	...
	public void carregarLaudoTecnico() {
		LaudoTecnicoEntity laudo = laudoTecnicoDAO.buscaLaudoTecnico(currentBean);
	...
	public void carregarObservacao() {
		ObservacaoEntity observacao = observacaoDAO.buscaObservacao(currentBean);
	...
	private void geraParcelasChequeOs(final List<OrdemServicoEfetivacaoEntity> parcelasCheque) {
		...
		TipoEfetivacaoEntity tipoEfetivacao = new TipoEfetivacaoEntity();
...
//			parcelaChequeDAO.delete(persistentObjects.get(i));
	...
	protected void remover(List<Serializable> ids) {
		ordemServicoDAO.deleteAllByIds(ids);
	...
	public List<InformacaoGeralEntity> buscarInformacaoGeral() {
		return informacaoGeralDAO.getAll(InformacaoGeralEntity.class);
	}
...
//		subView.preencheOsAgrupadaSubForm(osAgrupada);
	...
	public List<AcessorioEntity> buscarAcessorio() {
		return acessorioDAO.getAll(AcessorioEntity.class);
	}
...
//		currentBean.setValorServico(BigDecimal.ZERO);
	...
	public void getCarro(String classePesquisa, Integer idSelecionado){
		DefaultManyToOneComboModelSelect<CarroEntity> carro = new DefaultManyToOneComboModelSelect<CarroEntity>(CarroListController.class, this.carroDAO,
	...
	public ParametroOsEntity getParametroOs() {
		if(SecuritySessionProvider.getUsuario().getConta().getEmpresa()!=null){
			parametroOs = parametroOsDAO.buscaParametroOs(SecuritySessionProvider.getUsuario().getConta().getEmpresa());
		}
		return parametroOs;
	}
