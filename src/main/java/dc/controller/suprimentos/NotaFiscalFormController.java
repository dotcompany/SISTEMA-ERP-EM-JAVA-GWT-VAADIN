...
import dc.visao.suprimentos.NotaFiscalFormView;
import dc.visao.suprimentos.NotaFiscalFormView.CRT;
import dc.visao.suprimentos.NotaFiscalFormView.FINALIDADE_EMISSAO;
...
extends CRUDFormController<NotaFiscal>{
	...
	protected void actionSalvar() {
		try {
			...
			currentBean.setValorIcms(new BigDecimal(subView.getValorICMS().getValue()));
			currentBean.setBaseCalculoIcmsSt(new BigDecimal(subView.getBaseICMSST().getValue()));
			currentBean.setValorIcmsSt(new BigDecimal(subView.getValorICMSST().getValue()));
			currentBean.setValorCofins(new BigDecimal(subView.getTotalCofins().getValue()));
			currentBean.setValorTotalProdutos(new BigDecimal(subView.getTotalProdutos().getValue()));

			currentBean.setValorFrete(new BigDecimal(subView.getValorFrete().getValue()));
			currentBean.setValorSeguro(new BigDecimal(subView.getValorSeguro().getValue()));
			currentBean.setOutrasDespesasAcessorias(new BigDecimal(subView.getOutrasDespesas().getValue()));
			currentBean.setValorPis(new BigDecimal(subView.getValorTotalPIS().getValue()));
			currentBean.setValorDescontos(new BigDecimal(subView.getDescontos().getValue()));
			currentBean.setValorTotalNota(new BigDecimal(subView.getTotalNota().getValue()));
	...
	protected void carregar(Serializable id) {
		...
		subView.getDataEntradaSaida().setValue(currentBean.getDataEntradaSaida());
		subView.getHoraEntradaSaida().setValue(currentBean.getHoraEntradaSaida());
		subView.getNatureza().setValue(currentBean.getNaturezaOperacao());
		subView.getBaseICMS().setValue(currentBean.getBaseCalculoIcms().toString());
		subView.getValorICMS().setValue(currentBean.getValorIcms().toString());
		subView.getBaseICMSST().setValue(currentBean.getBaseCalculoIcmsSt().toString());
		subView.getValorICMSST().setValue(currentBean.getValorIcmsSt().toString());
		subView.getTotalCofins().setValue(currentBean.getValorCofins().toString());
		subView.getTotalProdutos().setValue(currentBean.getValorTotalProdutos().toString());
		subView.getValorFrete().setValue(currentBean.getValorFrete().toString());
		subView.getValorSeguro().setValue(currentBean.getValorSeguro().toString());
		subView.getOutrasDespesas().setValue(currentBean.getOutrasDespesasAcessorias().toString());
		subView.getValorTotalPIS().setValue(currentBean.getValorPis().toString());
		subView.getDescontos().setValue(currentBean.getValorDescontos().toString());
		subView.getTotalNota().setValue(currentBean.getValorTotalNota().toString());
