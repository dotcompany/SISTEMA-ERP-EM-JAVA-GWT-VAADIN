...
import dc.visao.framework.geral.ConfiguraNovaContaController;
...
public class FmModuloDAO extends AbstractCrudDAO<FmModulo>{
        ...
        public List<FmModulo> getAllOrderedByCaption() {  
                return getSession().createCriteria(FmModulo.class).setFetchMode("menus",FetchMode.SELECT).addOrder(Order.asc("caption")).list();
        }
        ...
        public List<FmModulo> getAllByUserIdOrderedByCaption(int user_id) {
                ...
                return getSession().createSQLQuery(q).addEntity(FmModulo.class).setInteger("user_id",user_id).list();
        }
        ...
        public List<FmModulo> getModulosSelecionaveis() {
                ...
                c.add(
                                Restrictions.not(
                                                Restrictions.in("id", new Integer[] {this.ID_MODULO_ADM_OBRIGATORIO,this.ID_MODULO_SISTEMA_OBRIGATORIO})
                                )
                );
        ...
        public List<FmModulo> getModulosObrigatorios() {
                Criteria c = getSession().createCriteria(FmModulo.class);
                c.add(Restrictions.in("id", new Integer[] {this.ID_MODULO_ADM_OBRIGATORIO,this.ID_MODULO_SISTEMA_OBRIGATORIO}) );
                return c.addOrder(Order.asc("caption")).list();
        }
