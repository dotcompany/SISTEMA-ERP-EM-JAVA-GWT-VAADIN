...
public abstract class AbstractCrudDAO<T> {
	...
	private void configureComboFields(Class<T> entityClass) {
		...
		java.lang.reflect.Field[] entityFields = entityClass
				.getDeclaredFields();
	...
	public void save(T obj) {
		if (obj instanceof AbstractMultiEmpresaModel) {
			...
			a.setEmpresa(SecuritySessionProvider.getUsuario().getConta()
					.getEmpresa());
		}
	...
	public List<T> getAllForCombo(final Class<T> type, int idEmpresa,
		...
		String order = comboValue.contains(".") ? comboValue.split("\\.")[0]
				: comboValue;
	...
	public List<T> fullTextSearch(String valor) {
		return fullTextSearch(valor, getSearchFields(), FIRST_ROW,
				DEFAULT_PAGE_SIZE, new String[0], new boolean[0], null, null);
	}
	...
	public List<T> fullTextSearch(String valor, int first, int pageSize,
			...
			List<Filter> filters) {
		return fullTextSearch(valor, getSearchFields(), first, pageSize,
				sortingFields, sortingStates, null, filters);
	}
	...
	public List<T> fullTextSearch(String valor, String[] sortingFields,
		...
		return fullTextSearch(valor, getSearchFields(), FIRST_ROW,
				DEFAULT_PAGE_SIZE, sortingFields, states, null, filters);
	}

	@Transactional
	private List<T> fullTextSearch(String value, String[] searchFields,
			int first, int pageSize, String[] sortingFieldsStrings,
		...
		org.apache.lucene.search.Query query = createQuery(value, searchFields,
				menu, filters, fullTextSession);

		FullTextQuery q = fullTextSession.createFullTextQuery(query,
				getEntityClass());
	...
	private org.apache.lucene.search.Query createQuery(String value,
		...
		if (isConsultaMultiEmpresa(getEntityClass(), menu)) {
			...
			query = createSimpleFullTextQuery(value, searchFields,
					fullTextSession, filters);
		}
	...
	@Transactional
	public int fullTextSearchCount(String searchValue, FmMenu menu,
			List<Filter> filters) {
		FullTextSession fullTextSession = getFullTextSession();

		org.apache.lucene.search.Query query = createQuery(searchValue,
				getSearchFields(), menu, filters, fullTextSession);
	...
	private org.apache.lucene.search.Query createSimpleFullTextQuery(
			String value, String[] searchFields,
	...
	private BooleanQuery createFieldQueryByValue(String value,
			String... searchFields) {
	...
	private org.apache.lucene.search.Query createMultiEmpresaQuery(
			String value, String[] searchFields, FmMenu menu,
			List<Filter> filters) {
		FullTextSession fullTextSession = getFullTextSession();
		org.apache.lucene.search.BooleanQuery booleanQuery = new BooleanQuery();

		org.apache.lucene.search.Query multiEmpresaQuery = buildMultiEmpresaQuery(
				value, searchFields, fullTextSession, menu);
	...
	private org.apache.lucene.search.Query generateFiltersQuery(
				...
				try {
					if (filter instanceof Compare) {
						...
						if (operation.equals(Compare.Operation.EQUAL)) {// EQUAL,
							...
							// property);
							query = createGreaterLessQuery(
									castedFilter.getValue(),
									castedFilter.getValue(), property);
						} else if (operation
								.equals(Compare.Operation.GREATER_OR_EQUAL)) {
							query = createGreaterLessQuery(
									castedFilter.getValue(), null, property);
						} else if (operation
								.equals(Compare.Operation.LESS_OR_EQUAL)) {
							query = createGreaterLessQuery(null,
									castedFilter.getValue(), property);
						...
						query = createGreaterLessQuery(
								castedFilter.getStartValue(),
								castedFilter.getEndValue(), property);
					} else if (filter instanceof SimpleStringFilter) {
						String property = ((SimpleStringFilter) filter)
								.getPropertyId().toString();
						String search = ((SimpleStringFilter) filter)
								.getFilterString();

						if (dc.control.validator.ObjectValidator
	...
	private org.apache.lucene.search.Query createDateQuery(Object startValue,
			Object endValue, Object property) {
		org.apache.lucene.search.Query query;
		// 20131001030000000
		String start = "*";
		String end = "*";
	...
	private org.apache.lucene.search.Query createGreaterLessQuery(
			Object startValue, Object endValue, Object property) {
		org.apache.lucene.search.Query query;
		// 20131001030000000
	...
	private org.apache.lucene.search.Query createEqualQuery(Object value,
			Object property) {
		FullTextSession fullTextSession = getFullTextSession();

		QueryBuilder qb = fullTextSession.getSearchFactory()
				.buildQueryBuilder().forEntity(getEntityClass()).get();
		TermTermination matching = qb.keyword().onField(property.toString())
				.matching(value);
	...
	private org.apache.lucene.search.Query buildMultiEmpresaQuery(String value,
			String[] searchFields, FullTextSession fullTextSession, FmMenu menu) {
	...
	@Transactional
	public List<T> fullTextSearch(String searchValue, int arg0, int arg1,
			String[] sortingFields, boolean[] sortStates, FmMenu menu,
			List<Filter> filters) {
		return fullTextSearch(searchValue, getSearchFields(), arg0, arg1,
				sortingFields, sortStates, menu, filters);
	}
		...
		if (defaultSearchFields == null || defaultSearchFields.length > 0) {
			...
			if (defaultSearchFields == null || defaultSearchFields.length == 0) {
				...
				defaultSearchFields = searchFields
						.toArray(new String[searchFields.size()]);
			}
		}
	...
	@Transactional
	public List<T> getAllPaged(Class<T> clazz, int start, int pageSize,
	...
	@Transactional
	public List<Serializable> getAllPagedByEmpresa(Class<T> pojoClass,
			Integer idEmpresa, int start, int pageSize, String[] sortingFields,
			boolean[] states) {
		Query query = sessionFactory.getCurrentSession().createQuery(
				"from " + pojoClass.getName()
						+ " where empresa.id = :id_empresa "
						+ getHQLOrderByClause(sortingFields, states));
	...
	public List<T> comboTextSearch(String value, FmMenu menu) {
		...
		org.apache.lucene.search.Query booleanQuery = createQuery(value,
				fields, menu, null, fullTextSession);
		resultSet = fullTextSession
				.createFullTextQuery(booleanQuery, getEntityClass())
